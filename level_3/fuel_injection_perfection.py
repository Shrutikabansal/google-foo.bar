...
import math
import math
def calculate_steps(n,lookup_table):
    if n in lookup_table:
        return lookup_table[n]
    if n & 1:
        lookup_table[n] = min(calculate_steps((n + 1) >> 1, lookup_table) + 2,
                                  calculate_steps((n - 1) >> 1, lookup_table) + 2)
    else:
        lookup_table[n] = calculate_steps(n >> 1, lookup_table) + 1
...
def solution(n):
   ...
   return calculate_steps(n,lookup_table)
