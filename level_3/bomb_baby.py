...
def solution(M, F):
    ans = 0
    array_mini_max = [int(M), int(F)]
    while array_mini_max[0] > 1 and array_mini_max[1] > 1:
        array_mini_max.sort()
        if array_mini_max[1] %  array_mini_max[0] == 0:
            return "impossible"
        ans += array_mini_max[1] // array_mini_max[0]
        array_mini_max[1]  = array_mini_max[1] %  array_mini_max[0]
    ans += max(array_mini_max) - 1
    return str(ans)
